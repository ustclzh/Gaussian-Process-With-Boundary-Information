...
for i=1:length(x)
wMax(i) = midpotin_deflection_simulator(x(i));
%-0.0138*pres*len^4./(E*(x).^3);
end
wMax=wMax';
%wMax_true=@(x) -0.0138*pres*len^4./(E*(x).^3);
%wMax = wMax_true(x);
wMax_b=midpotin_deflection_simulator(1);
...
y_p(i,:)=res(1,:);
sig(i,:)=res(2,:);
y_t(i)=midpotin_deflection_simulator(x_test(i));%wMax_true(x_test(i));
end
...
ylabel('Midpoint Deflection','FontSize',13,'Fontname', 'Times New Roman')
