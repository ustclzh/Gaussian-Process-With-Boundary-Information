function [alphaopt,etaopt,ropt,a0opt,s2,unscaledsigmaD,QI,QIRes,Minout,Rangeout,SDout,nout,kout,Condnum]=BMGPFit_ls(Din,Yin,Noin)
%checked 3
global D Min Range SD Y n No a k Dist2 IDist2 w1 w2 Index1 Temp_range logalpha_opt
No=Noin;
D=Din;
Y=Yin;
[n, d]=size(D); nout=n;
...
options=optimoptions(@fminunc,'MaxIter',10^6,'TolX',10^-6,'TolFun',10^-8,'MaxFunEvals',10^5,'Display','off','Algorithm','quasi-newton');
Sum1=min(w1);
Sum2=min(w2);
...
elseif(No==1)
    [logalpha_opt, fval1, exitflag1]=fminunc(@Obj_ms_reg,[log(startalpha)],options);  
    [paropt1, fval1, exitflag1]=fminunc(@Obj,[logstarteta log(0.91)*ones(1,d)],options);  
    [paropt2, fval2, exitflag2]=fminunc(@Obj,[logstarteta log(2.55)*ones(1,d)],options);      
elseif(No==2)
...
display('Optimal values of BMGP correlation parameters')
disp(ropt)
...
RI=invandlogdet(R);
alphaopt=exp(logalpha_opt(1));
sI=diag(1./unscaledsigmaD);
QI=sI*RI*sI;
Condnum=cond(invandlogdet(QI));
w3=alphaopt*IDist2;
w=w1+alphaopt*w2;
Lambda=zeros(n,k+1);
Lambda(:,1)=w1./w;
...
a0opt=(Lambda(:,1)'*Z)/(Lambda(:,1)'*Lambda(:,1));
Res=Z-Lambda(:,1)*a0opt;
QIRes=QI*Res;
s2=Res'*QIRes/n;

function Objective=Obj(par)
global SD Y n No a k Dist2 IDist2 w1 w2 Index1 logalpha_opt
if(sum(isnan(par))>0)
    Objective=Inf;
    return
end
alpha=exp(logalpha_opt(1));
eta=exp(par(1));
par=par((2):end);
...
elseif(No==1)
    ...
    if(eta<0.0000000000000000000000000001)
        Objective=Inf;
        return        
    end 
...
elseif(No==2)
    r=exp(par);
    if((sum(r<0.291731110468193)>0)||(sum(r>50)>0))
        Objective=Inf;
        return        
    end
end
...
function Objective=Obj_ms_reg(par)
global  Y n  a k  IDist2 w1 w2 Index1
...
Z=Y-sum(Lambda(:,2:end).*a,2);
a0opt=(Lambda(:,1)'*eye(n)*Z)/(Lambda(:,1)'*eye(n)*Lambda(:,1));
Res=Z-Lambda(:,1)*a0opt;
Objective=sum(Res.^2);
